...
class YOLOPAFPN(nn.Module):
    def __init__(self, depth = 1.0, width = 1.0, in_features = ("dark2","dark3", "dark4", "dark5"), in_channels = [256, 512, 1024,128], depthwise = False, act = "silu"):
        super().__init__()
        Conv                = DWConv if depthwise else BaseConv
        ...
        self.feat1_att = cbam_block(128)
        self.feat2_att = cbam_block(256)
        self.feat3_att = cbam_block(512)
        self.upsample1_att = cbam_block(256)
        self.upsample2_att = cbam_block(128)
        self.upsample3_att = cbam_block(64)
        self.downsample1_att = cbam_block(128)
        self.downsample2_att = cbam_block(256)
        self.downsample3_att = cbam_block(64)
        ...
        self.reduce_conv2   = BaseConv(int(in_channels[0] * width), int(in_channels[3] * width), 1, 1, act=act)
        #-------------------------------------------#
        #   80, 80, 512 -> 80, 80, 256
        #-------------------------------------------#
        self.C3_p3 = CSPLayer(
        ...
        # -------------------------------------------#
        self.bu_conv3 = Conv(int(in_channels[3] * width), int(in_channels[3] * width), 3, 2, act=act)
        # -------------------------------------------#
        #   80, 80, 256 -> 40, 40, 512
        # -------------------------------------------#
        self.C3_n2 = CSPLayer(
    ...
    #     # P3_downsample = self.downsample1_att(P3_downsample)
    #     #-------------------------------------------#
    ...
    def forward(self, input):
        ...
        P3_upsample = self.upsample3_att(P3_upsample)
        # -------------------------------------------#
        ...
        P2_downsample = self.downsample3_att(P2_downsample)
        # -------------------------------------------#
